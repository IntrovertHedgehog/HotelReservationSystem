...
public class AllocatingBotSessionBean implements AllocatingBotSessionBeanLocal {
    ...
    private ExceptionReport createException(Reservation reservation) {
        List<Room> allRooms = em.createQuery("SELECT r FROM Room r WHERE r.status <> :status")
                .setParameter("status", RoomStatus.DISABLE)
                .setParameter("checkInDate", reservation.getCheckInDate())
                .setParameter("checkOutDate", reservation.getCheckOutDate())
                .getResultList();
        
        List<Room> usedRooms = em.createQuery("SELECT r FROM Allocation a JOIN a.room r JOIN a.reservation re WHERE r.status <> :status AND re.checkInDate < :checkOutDate AND re.checkOutDate > :checkInDate")
                .setParameter("status", RoomStatus.DISABLE)
                .setParameter("checkInDate", reservation.getCheckInDate())
                .setParameter("checkOutDate", reservation.getCheckOutDate())
                .getResultList();
        ...
        try {
            Room room = allRooms.stream()
                    .filter(r -> r.getRoomType().getRoomTypeConfig().compareTo(reservation.getRoomType().getRoomTypeConfig()) >= 0
                            && !usedRooms.contains(r))
                    .findFirst()
